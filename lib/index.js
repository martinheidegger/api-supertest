...
function processItem(options, item, cb) {
	var state = {
		...
		function (cb) {
			state.request.end(function (err, res) {
				...
				if (item.json) {
					...
					item.json.validate(res.body, function (err, data) {
						if (err) {
							state.error = err;
						} else {
							passed += 1;
				...
				if (item.result) {
					if (typeof item.result === "function") {
						return item.result(res.body, cb);
					} else if (item.result !== res.body) {
...
function runTests(options, cb) {
	...
	async.series([
		...
		async.mapSeries.bind(async,
			options.tests,
			processItem.bind(null, options)
		),
		options.after.bind(options),
		function (cb) {
			process.nextTick(function () {
				isSuccess = passed === options.tests.length;
				options.output.end(passed, options.tests.length, cb); 
			});
		}
	], function () {
		if (cb) {
