...
const dataDefinition = joi.string()
...
const optionsDefinition = joi.object({
  ...
  before: joi.func().optional().default(statsPassthrough),
  after: joi.func().optional().default(statsPassthrough),
  beforeEach: joi.func().optional().default(statePassthrough),
  afterEach: joi.func().optional().default(statePassthrough),
...
function prepareRequest (base, state, cb) {
  process.nextTick(function () {
    const item = state.item
    const r = new supertest.Test(base, item.method, item.path)
    if (item.code) {
      r.expect(item.code)
    }
    if (item.data) {
